...
$PS1URL = "https://raw.githubusercontent.com/RonRunnerElowSum/WindowsUpdate/Prod-Branch/Install%20Windows%20Updates%20(PRODUCTION%20group).ps1"
...
if(Get-ScheduledTask -TaskName "(MSP) Install Missing Windows Updates (PatchGroup -- PRODUCTION)" -ErrorAction SilentlyContinue){
    Write-Host "The scheduled task [(MSP) Install Missing Windows Updates (PatchGroup -- PRODUCTION)] already exists...exiting..."
    EXIT
}
CreateSchedTask
