...
function PunchIt () {
    ...
    if($PatchGroupTest | Select-String $CurrentDate){
        ...
        Write-MSPLog -LogSource "MSP Patch Health" -LogType "Information" -LogMessage "Outside of patch window...$Env:COMPUTERNAME patches on the following days this patch cycle:`r`n`r`n$PatchGroupTest"
        EXIT
    }
}
