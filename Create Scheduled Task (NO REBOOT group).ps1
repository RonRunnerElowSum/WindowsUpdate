$TaskName = "(MSP) Install Missing Windows Updates (PatchGroup -- NO REBOOT)"
$PS1URL = "https://raw.githubusercontent.com/RonRunnerElowSum/WindowsUpdate/Prod-Branch/Install%20Windows%20Updates%20(NO%20REBOOT%20group).ps1"
...
function CreateSchedTask () {
    ...
    Register-ScheduledTask -Action $Action -Trigger $Trigger -Settings $Settings -User SYSTEM -TaskName $TaskName -Description "Keeps Windows up-to-date." | Out-Null
...
if(Get-ScheduledTask -TaskName "(MSP) Install Missing Windows Updates (PatchGroup -- NO REBOOT)" -ErrorAction SilentlyContinue){
    Write-Host "The scheduled task [(MSP) Install Missing Windows Updates (PatchGroup -- NO REBOOT)] already exists...exiting..."
    EXIT
}
CreateSchedTask
